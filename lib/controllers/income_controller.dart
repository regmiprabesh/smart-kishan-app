...
class IncomeController extends GetxController {
  ...
  final RxList<Activity> incomeActivities = <Activity>[].obs;
  ...
  /// Sums income for every activity that falls into each bucket, returning
      ...
      for (var i = 0; i < bucketKeys.length; i++)
        FlSpot(
          ...
          (grouped[bucketKeys[i]] ?? const <Activity>[])
              .fold<double>(0.0, (sum, a) => sum + (a.income ?? 0)),
        ),
    ];
  }
  ...
  Future<void> getActivities(String filter) async {
    try {
      ...
      incomeActivities.assignAll(income);
      ...
      final spots = _plot(income, keys, keyFmt);
      plots.assignAll(spots);
      _updatePlotBounds(keys, spots);
    } catch (e) {
      debugPrint('IncomeController.getActivities error: $e');
    }
  }
}
